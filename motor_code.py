...
def user_interface(film_motor, takeup_motor, settings):
    ...
    frames = int(frames * .99)

    try:
        while True:
            if frames == settings['index']:
                break
            settings = new_index(settings)
            take_picture(film_motor, takeup_motor, camera, settings)
...
def new_index(settings):
    settings['image'] = f'{settings["movie_folder"]}/{"{:05d}".format(settings["index"])}{settings["file_extension"]}'
    settings['index'] += 1
